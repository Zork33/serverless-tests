 ...
 */
export const setTestInitializeTimerAdvance = (
  timerAdvance: (duration: number) => void,
) => {
 ...
 * **Only for unit tests purposes**.
 */
export const getTestInitPromise = (): Promise<string | void> | undefined =>
  testInitPromise;
...
export class MetadataTokenService implements TokenService {
  ...
  constructor(
    urlOrOptions: string | MetadataTokenService.Options = DEFAULT_URL,
    options: MetadataTokenService.Options = {},
  ) {
    ...
    const { logger, doUpdateTokenInBackground, ...opts } = options;
    ...
    this.logger.debug(
      Messages.debug_ctor,
      this.url,
      this.doUpdateTokenInBackground,
      this.axiosOptions,
    );

    if (this.doUpdateTokenInBackground) {
      testInitPromise = this.getToken().catch(() => {}); // intentionally without await
    }
  }
  ...
  async getToken(): Promise<string> {
    ...
    if (!this.currentGetTokenPromise) {
      this.currentGetTokenPromise = this._getToken().finally(() => {
  ...
  setIamToken(iamResponse: MetadataTokenService.IamToken) {
    ...
    const TTL =
      iamResponse.expires_in * 1000 - TOKEN_MINIMUM_LIFETIME_MARGIN_MS;
    ...
    this.tokenRefreshAt =
      Date.now() +
      iamResponse.expires_in *
        1000 *
        (1 - TOKEN_LIFETIME_LEFT_TO_REFRESH_PCT / 100);
    this.tokenStartReportTTLAt =
      Date.now() +
      iamResponse.expires_in *
        1000 *
        (1 - TOKEN_LIFETIME_LEFT_TO_REPORT_ERROR_PCT / 100);
  ...
  private async initialize() {
    ...
    for (let i = 1; ; i++) {
      ...
      const duration = Math.trunc(
        maxDuration * (1 - Math.random() * INITIALIZE_BACKOFF_UNCERTAIN_RATIO),
      );
      ...
      await new Promise(async (resolve, reject) => {
        try {
          ...
          if (testInitializeTimerAdvance)
            await testInitializeTimerAdvance(duration);
        } catch (error) {
          /* istanbul ignore next */
          reject(error);
        }
  ...
  private async fetchToken(): Promise<string> {
    ...
    try {
      ...
      const res = await axios.get<MetadataTokenService.IamToken>(
        this.url,
        this.axiosOptions,
      );
      ...
      if (res.status !== 200) {
        throw new Error(
          util.format(Messages.err_part_http_error, res.status, res.statusText),
        );
      }
      ...
      // eslint-disable-next-line no-bitwise
      const slotsCount =
        1 << Math.min(this.tokenErrorCount++, GET_TOKEN_BACKOFF_CEILING);
      const maxDuration = slotsCount * GET_TOKEN_BACKOFF_SLOT_DURATION;

      this.tokenRefreshAt =
        Date.now() +
        maxDuration * (1 - Math.random() * GET_TOKEN_BACKOFF_UNCERTAIN_RATIO);
