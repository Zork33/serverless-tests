...
const isOAuth = (config: SessionConfig): config is OAuthCredentialsConfig =>
  'oauthToken' in config;

const isIamToken = (
  config: SessionConfig,
): config is IamTokenCredentialsConfig => 'iamToken' in config;

const isServiceAccount = (
  config: SessionConfig,
): config is ServiceAccountCredentialsConfig => 'serviceAccountJson' in config;
...
const createIamToken = async (
  ...
  const client = clientFactory.create(
    serviceClients.IamTokenServiceClient.service,
    channel,
  );
...
const newTokenCreator = (config: SessionConfig): (() => Promise<string>) => {
  if (isOAuth(config)) {
    return () => {
      const iamEndpoint = getServiceClientEndpoint(
